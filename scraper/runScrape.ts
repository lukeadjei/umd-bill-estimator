...
// The actual scrape-everything-into-staging logic, extracted out of the CLI
// entry point (index.ts) so it has exactly one home -- both the local
// `npm run scrape` command and the Lambda handler call this same function
// rather than each having their own copy that could drift out of sync.
// Sequential, not parallel: if one page's structure has changed and
// extraction fails, we want to know immediately, not after writing five
// other groups' worth of data.
export async function runScrape(academicYearId: string): Promise<void> {
