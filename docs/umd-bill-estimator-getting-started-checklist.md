...
- [ ] **Auth page** (`/sign-in` or similar) — screen/layout started 2026-09-02, Google OAuth only for now (see Decisions log for reasoning). Still open: the actual Supabase Google OAuth wiring (Google Cloud Console client + Supabase provider config, both deferred until the screen itself is done), and deciding whether email/password gets added later as a second option.
